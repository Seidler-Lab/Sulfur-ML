"""Adapted from Will Holden's tddftoutputparser."""
import numpy as np
...
def linear_broaden_from_transitions(transitions, lorentz_ev_start=0.6, numpoints=1000, peaknorm=True):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-eshift', action='store_true', default=False, dest='eshift', 
    ...
    if result.mode == 'XES':
        ...
        # xanes
        transitions = read_tddft_transitions_file(result.filename)
        xes_trans = read_tddft_transitions_file(f"xes_{result.filename}")
        transitions = transition_normalize_xanes(transitions, xes_trans)

    # energy shift, for XES: 18.6, for XANES: -53.3
    # to match experimental data
    if result.eshift:
        if result.mode == 'XES':
            # from Holden et al
            transitions = energy_shift(transitions, 18.6)
        elif result.mode == 'XANES':
            # from matching dibenzyl sulfide in
            # Functional Groups and Sulfur K-Edge XANES Spectra:
            # Divalent Sulfur and Disulﬁdes, Mijovilovich
            transitions = energy_shift(transitions, -53.3)    
    ...
    # normalize spectra (only if not normalizing transitions first)
